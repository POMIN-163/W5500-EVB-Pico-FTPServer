...
void LCD_SetRotation(u8 rot)
{
    ...
    // Memory data access control (MADCTL)
    LCD_WR_REG(0x36);
    if (_rotation == 0)LCD_WR_DATA8(0x50 | (RGB_ORDER << 3));
    else if (_rotation == 1)LCD_WR_DATA8(0x80 | (RGB_ORDER << 3));
    else if (_rotation == 2)LCD_WR_DATA8(0x20 | (RGB_ORDER << 3));
    else LCD_WR_DATA8(0xe0 | (RGB_ORDER << 3));
}
...
void LCD_ShowPicture(u16 x1, u16 y1, u16 x2, u16 y2, u8 *image)
{
    ...
    for (j = y1; j < y2; j++)
    {
        for (i = x1; i < x2; i++)
        {
            LCD_WR_DATA8(image[0]);
            LCD_WR_DATA8(image[1]);
            image += 2;
        }
    }
}
